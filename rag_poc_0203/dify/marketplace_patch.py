...
def batch_fetch_plugin_manifests(plugin_ids: list[str]) -> Sequence[MarketplacePluginDeclaration]:
    ...
    response = httpx.post(url, json={"plugin_ids": plugin_ids}, headers={"X-Dify-Version": dify_config.project.version}, verify=False)
...
def record_install_plugin_event(plugin_unique_identifier: str):
    ...
    response = httpx.post(url, json={"unique_identifier": plugin_unique_identifier}, verify=False)
    response.raise_for_status()
